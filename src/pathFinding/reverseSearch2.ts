...
func = (
		...
		if (cpRoutes?.length > 0 && cpRoutes[0].points?.length > 0) {
			...
			for (let branch of cpRoutes) {
				...
				const intersectPoint = {
					...branchOutwards.points[0],
					start: !branchOutwards.points[0].start
				};
	...
	function getRouteTo(from: PathNode, to: PathNode[], reverse: boolean = false): Route[] {
		...
		function searchNextPoint(route: Route, depth: number = 0) {
			...
			usedPoints[depth] = usedPoints[depth]
				? usedPoints[depth].concat(curNode.paths)
				: curNode.paths;

			const nextPoints = curNode.paths.map((p) => {
				return { ...p, start: !p.start };
			});
			const wrongWay = (p: PathNode) => paths[p.index]?.type == 'oneway' && p.start == !reverse;
			const hasBeenVisited = (p: PathNode) =>
				usedPoints
					.slice(0, depth + 1)
					.flat()
					.some((u) => u.index == p.index && u.start == p.start);
	...
	function joinBranch(
		...
		const connectBackPoint =
			nextCpIndex > -1 ? segment2[nextCpIndex] : root.points[root.points.length - 1]; //finish if not on segment1
	...
	function findPointIndexes(points: PathNode[], targetPoint: PathNode) {
		...
		for (let i = 0; i < points.length; i++) {
			const p = points[i];
			if (isSameNode(p, targetPoint)) indexes.push(i);
		}
		return indexes;
	}
	...
	function findLastTakenCPIndex(
		points: PathNode[],
		selectCps: number[] = undefined,
		filter = (cp: Route['cps'][0]) => cp.num > -1
	) {
		const nodes = points.map((p) => pointToNode(p));
		const takenCps = pointsToTakenCps(points);

		const lastCPNum = takenCps
			.slice()
			.reverse()
	...
	function findFirstTakenCPIndex(
		points: PathNode[],
		selectCps: number[] = undefined,
		filter = (cp: Route['cps'][0]) => cp.num > -1
	) {
		...
		const firstCPNum = takenCps.find(
			(cp) => (!selectCps || selectCps.includes(cp.num)) && filter(cp)
		)?.num; //find num of the first taken CP
